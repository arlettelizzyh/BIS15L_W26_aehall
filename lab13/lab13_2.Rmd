...
```
```{r}
anti_join(table_B, table_A, by="customer_ID")
```
...
```{r}
ymd(dateformat1) #these functions usually used within mutate()
```
...
```{r message=FALSE, warning=FALSE}
spider_records <- read_csv("~/Desktop/datascibiol-master/lab13/data/spider_records.csv") %>%
  clean_names()
```

```{r, message = FALSE}
spider_locations <- read_csv("~/Desktop/datascibiol-master/lab13/data/spiders_locations.csv") %>%
  clean_names()
```

2. Join the files using an appropriate key and join type.
```{r}
anti_join(spider_records, spider_locations, by="accession")
anti_join(spider_locations, spider_records, by="accession")
```

```{r}
spider_all <- full_join(spider_records, spider_locations, by="accession")
```

```{r}
names(spider_all) #check work
```
...
```{r}
write.csv(spider_all, "~/Desktop/datascibiol-master/lab13/data/spider_all.csv", row.names=F)
```
