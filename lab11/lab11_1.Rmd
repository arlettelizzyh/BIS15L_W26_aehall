...
gap %>%
  ...
  geom_line(na.rm=T)+
  geom_point(shape=5)+
  scale_x_continuous(breaks=2000:2025, minor_breaks = NULL)+ #x-axis, show all #'s
  theme(axis.text.x = element_text(angle=60, hjust=1))+
  labs(x="Year", y="GDP", title="USA: GDP per Capita, 2000-2025")
```
...
gap %>%
  filter(year==1935) %>%
  ggplot(aes(x=lex))+
  geom_histogram(color="black", fill="turquoise4", bins=30, na.rm=T)+
...
gap %>%
  ...
  geom_density(color="turquoise4", fill="turquoise4", alpha=0.4, na.rm=T)+
...
gap %>%
  ...
  labs(x="Life Expectancy (yrs)", y=NULL,
       title="Distribution of Life Expectancy, 1930 & 1950",
       color="Year", fill="Year")  #fix legend
```
...
```{r}
income_cat2000 %>%
  #can also do: drop_na(income_category) %>%
  filter(income_category != "NA") %>%
...
income_cat2000 %>%
  filter(income_category != "NA") %>%
  group_by(income_category) %>%
  ggplot(mapping=aes(x=income_category, fill=income_category))+
  geom_bar(color="black", alpha=0.8)+
  labs(x="Income Category",
       y=NULL,
       title="Number of Countries per Income Category, 2000")+
