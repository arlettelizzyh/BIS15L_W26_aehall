...
Height_m <- c(8848.86, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble(Names, Height_ft, Height_m)
mountains
```

**3. What is the mean height of the mountains in feet?**
```{r}
mean_height_ft <- mean(mountains$Height_ft)
mean_height_ft
```
...
```{r}
summits <- c(12884, 800, 400, 1100, 560)
summits
```
