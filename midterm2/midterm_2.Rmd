...
#  group_by(year, popn) %>%
#  summarize(n_count = n(), .groups='keep') %>%
#  ggplot(aes(x=year, y=n_count))+
#  geom_line(color=popn)+
#  labs(x="Year", y=NULL,
#       title="Count of Polar Bears by Population and Year",
#       color="Population")+
#  theme(plot.title=element_text(size=rel(1.2), hjust=0.5, face="bold"),
#        axis.title.x=element_text(face="bold")) %>%
#  scale_x_continuous(breaks=seq(1983, 2016, by=10))
```
...
#  ggplot(aes(x=year, y = n_fast, color=popn))+
#  geom_point(na.rm=T)+
...
#             aes(long, lat), size=0.5, color="black", shape=popn, alpha=0.6)+
