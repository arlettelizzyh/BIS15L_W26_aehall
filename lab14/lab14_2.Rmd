...
```{r}
#check as boring plot before making interactive plot
penguins %>%
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data=penguins,
           aes_string(x=input$x, y=input$y, color="species", shape="sex"))+
      geom_point(na.rm=T)
  })
}
...
```{r, eval=F, echo=T}
#syntax that R wants to change to overtime
library(shiny)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(penguins,
#RIGHT HERE, upcoming syntax change is below
           aes(x = .data[[input$x]],
               y = .data[[input$y]],
               color = .data[["species"]],
               shape = .data[["sex"]]))+
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
homerange <- read_csv("~/Desktop/BIS15L_W26_aehall/lab14/data/Tamburelloetal_HomeRangeDatabase.csv") %>% clean_names()
```
