...
```{r}
#zoom number is how detailed the map is (higher number = higher res)
...
```{r}
#size is size of the dot
ggmap(map1)+
  geom_point(data=spiders, aes(longitude, latitude), size=0.5, color="black", alpha=0.8)+
...
```{r message=FALSE, warning=FALSE}
grizzly <- read_csv("~/Desktop/BIS15L_W26_aehall/lab14/data/bear-sightings.csv")
```
...
```
```{r}
long <- c (-166.2336, -131.2621)
lat <- c(55.01637, 70.36742)
bbox <- make_bbox(long, lat, f=0.03)
```
...
```{r}
ggmap(map1)+
  geom_point(data=grizzly, aes(longitude, latitude), size=0.5, color="black", alpha=0.8)+
