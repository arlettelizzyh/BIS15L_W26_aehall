...
elephants %>%
  ...
  summarize(mean_age=mean(estimated_age_years, na.rm=T),
    median_age=median(estimated_age_years, na.m=T),
...
msleep %>%
  group_by(vore) %>%
  summarize(min_bodyweight=min(bodywt),
...
msleep %>%
  group_by(order) %>%
  summarize(mean_brainweight=mean(brainwt))
```
...
msleep %>%
  group_by(order) %>%
  summarize(mean_brainweight=mean(brainwt, na.rm=T))
```
