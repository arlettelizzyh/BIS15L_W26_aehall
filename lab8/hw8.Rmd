...
```{r}
fisheries_clean %>%
  summarize(no_countries=n_distinct(country))
...
```{r}
fisheries_clean %>%
  summarize(no_species=n_distinct(common_name))
#There are 3,390 species when using common_name variable.
```

```{r}
fisheries_clean %>%
  summarize(no_species=n_distinct(taxonomic_code))
#There are 3,722 species when using taxonomic_code variable.
#So, no the number of species is not the same for the two variables.
```
