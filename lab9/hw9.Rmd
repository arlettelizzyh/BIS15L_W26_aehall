...
#Each column is a type of variable. Each row is its own observation. Each cell is its own value.
#The data's in long format.
```
...
```{r}
#Is the same beach the most polluted year-to-year or does it vary?

sydneybeaches_long %>% #check 2013
  group_by(year, site) %>%
  summarize(avg_enterococci_cfu_100ml = mean(enterococci_cfu_100ml, na.rm=T,),
            n = n(),
            .groups='keep') %>%
...
```{r}
#Which beach is the most polluted varies year by year. However, Malabar Beach is number 1 for 4 out of 6 years, and never leaves the top 5.
```
